...
def sign_image_url(code: str, ttl_seconds: int | None = None) -> tuple[str, int]:
    ...
    return f"/i/{code}?expires={expires}&sig={_signature(code, expires)}", expires


def build_signed_image_url(
    request: Request, code: str, ttl_seconds: int | None = None
) -> tuple[str, int]:
    """Return ``(absolute_signed_url, expires_unix_ts)``."""
    signed_path, expires = sign_image_url(code, ttl_seconds)
    if settings.public_url:
        base = settings.public_url.rstrip("/")
    else:
        base = str(request.base_url).rstrip("/")
...
def verify_image_signature(code: str, expires: str, sig: str) -> bool:
    ...
    if time.time() > expires_ts:
        return False
    expected = _signature(code, expires_ts)
